...
def intent_get_aula(entities):
    ...
    try:   
        ...
        data = datetime.today().replace(hour= 0)
        text_day = "Oggi"
    ...
    try:
        if(data.strftime("%H") == "00"):
            ...
            sql = "SELECT aula, corso, orario_inizio from lezioni WHERE giorno = %s AND corso REGEXP "+corso+"ORDER BY orario_inizio"
            orario_inizio = data.strftime("%H:%M")
            s.execute(sql, (dizionario[data.strftime('%A')], ))
        else:        
            corso = "'" + entities["corso:corso"][0]["value"] +"'"
            sql = "SELECT aula, corso, orario_inizio from lezioni WHERE orario_inizio <= %s AND orario_fine > %s AND giorno = %s AND corso REGEXP "+corso+"ORDER BY orario_inizio"
            orario_inizio = data.strftime("%H:%M")
            s.execute(sql, (orario_inizio,orario_inizio, dizionario[data.strftime('%A')], ))
    except:
        if(data.strftime("%H") == "00"):
            sql = "SELECT aula, corso, orario_inizio from lezioni WHERE giorno = %s ORDER BY orario_inizio"
            orario_inizio = data.strftime("%H:%M")
            s.execute(sql, (dizionario[data.strftime('%A')]))
        else:
            sql = "SELECT aula, corso, orario_inizio from lezioni WHERE orario_inizio <= %s AND orario_fine > %s AND giorno = %s ORDER BY orario_inizio"
            orario_inizio = data.strftime("%H:%M")
            s.execute(sql, (orario_inizio, orario_inizio, dizionario[data.strftime('%A')], ))
        ...
        if(len(res) > 0):
            if(data.strftime("%H") == "00"):
                ...
                for i in range (0, len(res)):
                    response += res[i][1]+" nell'aula "+res[i][0]+" dalle "+str(res[i][2])[:-3]+" "
            else:
                response = text_day + " hai lezione di: " + res[0][1] +" nell'aula " + res[0][0] + " dalle " +str(res[0][2])[:-3]+", "
            return response         
        else:
...
def intent_get_corso(entities):
    ...
    if("settimana" in text_day):
        sql = "SELECT corso, giorno from lezioni ORDER BY giorno, orario_inizio"
        try:
        ...
        try:
            orario_inizio = data.strftime("%H:%M")
            if(data.strftime("%H") == "00"):
                sql = "SELECT corso from lezioni WHERE  giorno = %s ORDER BY orario_inizio"
                s.execute(sql, (dizionario[data.strftime('%A')]))
            else:
                sql = "SELECT corso from lezioni WHERE orario_inizio <= %s AND orario_fine > %s AND giorno = %s ORDER BY orario_inizio"    
                s.execute(sql, (orario_inizio, orario_inizio, dizionario[data.strftime('%A')],))
            res = s.fetchall()  
            if(len(res) > 0):
                if(data.strftime("%H") == "00"):
...
def intent_get_orario(entities):
    ...
    try:
        if("settimana" in text_day):
            corso = "'" + entities["corso:corso"][0]["value"] +"'"
            sql = "SELECT  corso, orario_inizio, giorno from lezioni WHERE corso REGEXP "+corso+" ORDER BY giorno,orario_inizio"  
            ...
            try:
                ...
                if(len(res) > 0):
                    response = text_day+ " hai lezione di "+res[i][0]+" il: "
                    for i in range (0, len(res)):
                        response += str(res[i][2])+" alle "+str(res[i][1])[:-3]+", "          
                else:
                    response = "Non ci sono lezioni"
                return response       
            except:
                return "Errore connessione"
        elif(data.strftime("%H") != "00"):
            return intent_get_corso(entities)
        else:        
            corso = "'" + entities["corso:corso"][0]["value"] +"'"
            sql = "SELECT  corso, orario_inizio, orario_fine from lezioni WHERE giorno = %s AND corso REGEXP "+corso+" ORDER BY orario_inizio"  
            #stampo i risulati della query          
            try:
                s.execute(sql, (dizionario[data.strftime('%A')]))
                res = s.fetchall()
                if(len(res) > 0):
                    response = text_day+ " hai lezione di: "
                    for i in range (0, len(res)):
                        response += res[i][0]+" dalle "+str(res[i][1])[:-3]+" alle "+str(res[i][2])[:-3]          
                else:
                    response = "Non ci sono lezioni"
                return response       
            except:
        ...
        sql = "SELECT corso, orario_inizio from lezioni WHERE giorno = %s ORDER BY orario_inizio"
        ...
        try:
            ...
            try:
                ...
                res = s.fetchall()
                if(len(res)>0):
                    response = text_day + " le lezioni iniziano alle: "+str(res[0][1])[:-3]
                else:
                    response = "Non ci sono lezioni"
                return response        
            except:
            ...
            try:
                ...
                sql = "SELECT corso, orario_fine from lezioni WHERE giorno = %s ORDER BY orario_inizio"
                try:
                    s.execute(sql, (dizionario[data.strftime('%A')]))
                    res = s.fetchall()
                    if(len(res)>0):
                        response = text_day + " le lezioni finiscono alle: "+str(res[len(res)-1][1])[:-3]
                    else:
                        response = "Non ci sono lezioni"    
                    return response    
                except:
                ...
                try:
                    if("settimana" in text_day):
                        sql = "SELECT corso, giorno, orario_inizio, orario_fine from lezioni ORDER BY giorno,orario_inizio"
                        s.execute(sql)
                        res = s.fetchall()
                        if(len(res) > 0):
                            day_week = " il "+str(res[0][1])+" di: "
                            ...
                            for i in range(0, len(res)):
                                if(i != 0 and res[i][1] != res[i-1][1]):
                                    response += " il "+str(res[i][1])+" di: "
                                response += " "+str(res[i][0])+" dalle "+str(res[i][2])[:-3]+" alle "+str(res[i][3])[:-3]               
                        else:
                            response = "Non ci sono lezioni"
                        return response
                    else:   
                        ...
                        if(len(res) > 0):
                            response = text_day+ " hai lezione di: "
                            for i in range (0, len(res)):
                                response += res[i][0]+" alle "+str(res[i][1])[:-3]+", "               
                        else:
                            response = "Non ci sono lezioni"
                        return response    
                except:
...
def intent_get_quante_ore(entities):
    ...
    if("settimana" in text_day):
        ...
        try:
            corso = "'" + entities["corso:corso"][0]["value"] +"'"
            sql = "SELECT corso, orario_inizio, orario_fine from lezioni WHERE corso REGEXP "+corso+"ORDER BY giorno, orario_inizio"
            text_corso = entities["corso:corso"][0]["value"]
        except:
            sql = "SELECT corso, orario_inizio, orario_fine from lezioni ORDER BY orario_inizio"
        end_data = data + timedelta(days = 7)
        s.execute(sql)
        res = s.fetchall()
        print(res)
        ...
        try:
            ...
            sql = "SELECT corso, orario_inizio, orario_fine from lezioni WHERE giorno = %s ORDER BY orario_inizio"
        s.execute(sql, (dizionario[data.strftime('%A')]))
        res = s.fetchall()
    print(res)    
    ...
    if(tot_ore == timedelta(hours=0)):
        ...
        if(text_corso != None):
            response = text_day + " hai "+ str(tot_ore)[:-3] + " ore di " + text_corso
        else:
            response = text_day + " hai "+ str(tot_ore)[:-3] + " ore di lezione"
    return response          
...
def db_response(input):
        ...
        if(intent == None or confidence <= 0.65):
